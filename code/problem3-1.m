...
% 分析干涉条纹周期性
    ...
    if N > 10
        ...
        % 寻找主频率
        valid_range = 2:floor(N/2);
        if ~isempty(valid_range) && length(valid_range) > 1
            [~, max_freq_idx] = max(power_spectrum(valid_range));
            max_freq_idx = max_freq_idx + 1;  % 调整索引
            
            dominant_frequency = max_freq_idx / N;
            spectral_purity = power_spectrum(max_freq_idx) / sum(power_spectrum(valid_range));
        else
...
% 分析相位相干性
    ...
    instantaneous_phase = atan2(imag(analytic_signal), real(analytic_signal));
