...
% 保存为EPS文件
print(gcf, 'figures/reflectance_spectrum.eps', '-depsc', '-r300');
close(gcf);

fprintf('图2: 碳化硅晶圆片反射光谱数据已保存为 figures/reflectance_spectrum.eps\n');
end
...
% 保存为EPS文件
print(gcf, 'figures/phase_difference_analysis.eps', '-depsc', '-r300');
close(gcf);

fprintf('图3: 相位差分析结果已保存为 figures/phase_difference_analysis.eps\n');
end